...
- OCI 基盤（infra-oci リポジトリが管理する Kubernetes クラスタ・MySQL・共有 Ingress）。
  ...
  実績管理アプリ（コンテナイメージ・アプリケーションコード）のみを管理する
...
閲覧者のブラウザは、ポートフォリオサイトの静的ファイルを GitHub Pages から取得したうえで、
表示データ（スキル・資格・実績）を実績管理アプリの読み取り専用 API から取得する。
案件情報を含む登録操作は本人のログインを要する画面からのみ行う。

実績管理アプリ・実績 DB の実体（Kubernetes・MySQL）は infra-oci リポジトリが管理する OCI 基盤上に
...
- 実績管理アプリは、infra-oci 基盤上の既存アプリ（kakeibo）と技術スタックを揃え、
...
実績管理アプリ（`webapp/`）の言語・フレームワーク・バージョンは
...
| Skills スキルバー | 実績管理アプリの読み取り専用 API（[6.2](#62-ポートフォリオ用読み取り専用api)）を `fetch` で呼び出し、カテゴリ別にグルーピングして描画する |
...
| アクセス制御 | 登録・変更操作は本人のログインを要する画面からのみ実行できる（[6.4](#64-実績管理アプリ)） |
| 案件実績登録 | 案件の開始年月・終了年月（継続中の場合は未設定）とスキル項目のチェック状態を、その案件の実績として登録・更新する（[5.3.2](#532-スキル実績管理実績管理アプリ)） |
| 直近案件の初期表示 | 登録画面の初期状態として、直近案件（終了年月が未設定の案件。なければ終了年月が最も新しい案件）の登録内容を表示する（[5.3.2](#532-スキル実績管理実績管理アプリ)） |
| スキル項目のページ送り登録 | 案件登録画面のスキル項目チェックを `category` 単位のページへ分割し、次のカテゴリへ進める形式で表示する。パブリッククラウドの各提供元（AWS・Azure・OCI 等）も `category` の一つとして同じ仕組みでページ分割される（[5.3.2](#532-スキル実績管理実績管理アプリ)） |
| スキル項目の管理 | マスタ（`skills`）の追加・変更・削除を画面から行う。実績が紐づく項目は削除できない（[5.3.2](#532-スキル実績管理実績管理アプリ)） |
| Certifications 資格一覧 | 実績管理アプリの読み取り専用 API（[6.2](#62-ポートフォリオ用読み取り専用api)）を `fetch` で呼び出し、`js/main.js` が `#certifications-container` に描画する |
| Works 実績カード | 実績管理アプリの読み取り専用 API（[6.2](#62-ポートフォリオ用読み取り専用api)、`title` / `desc_ja` / リンク等）を `fetch` で呼び出し、`js/main.js` が `#works-container` に描画する |
| 資格・実績の管理 | 実績管理アプリの登録画面から資格（`certifications`）・実績（`works`）の追加・変更・削除を行う（[5.3.2](#532-スキル実績管理実績管理アプリ)） |
...
`certifications`・`works` は `skills`・`projects` と参照関係を持たない独立したテーブルとし、
...
| `level` | 整数 | ○ | 習熟度レベル。`levels` の `level` を参照する |
| `remarks` | 文字列 | - | 職務経歴書のレベル列に付記する補足 |
...
`name` とするスキル項目は持たない。クラウドの提供元単位で経験年数・習熟度を管理する上位項目は
...
削除前に画面上で確認を求める（[5.3.2](#532-スキル実績管理実績管理アプリ)）。
...
#### 5.1.2. スキル実績管理（実績管理アプリ）

| 画面 | 内容 |
| ---- | ---- |
| ログイン | 本人のみがログインできる認証画面 |
| 案件登録 | 継続中の案件（なければ新規案件）の開始年月・終了年月とスキル項目をチェックして案件実績を保存する。削除もここで行う。ログイン後の初期表示画面とする |
| スキル項目管理 | スキル項目マスタ（`skills`）の一覧・追加・変更・削除 |
| 資格・実績管理 | `certifications`・`works` の一覧・追加・変更・削除 |
...
```mermaid
flowchart TD
    Start["実績管理アプリへアクセス"] --> Login["ログイン"]
    Login --> Project["案件登録"]
    Project <--> Skill["スキル項目管理"]
    Project <--> CertWork["資格・実績管理"]
...
[5.3.2](#532-スキル実績管理実績管理アプリ) の規則に従いページ送りで進行する。「戻る」「次へ」に
...
| ページタブ | 「案件情報」「カテゴリ名（カテゴリ数だけ表示）」「確認」の各ページ名を横に並べたタブ。クリックすると入力内容を保持したまま該当ページへ直接移動する。既存の「戻る」「次へ」ボタンと併用できる |
| 案件情報ページ | `name`、`start_year_month`、`end_year_month`（継続中の場合は空欄のままにするチェックを設ける）を入力する。ページ送りの最初のページとする |
| カテゴリ別スキル項目ページ | `skills` に存在する `category` ごとに 1 ページを割り当て、そのカテゴリのスキル項目を `sort_order` 順のチェックボックスで表示する。その案件で使用したスキル項目をチェックし、「次へ」で次のカテゴリのページへ進む。パブリッククラウドの各提供元（AWS・Azure・OCI 等）も `category` の一つのため、他のカテゴリと同じ形式でページが割り当てられる。チェックした項目にはバージョン入力欄を表示し、使用したバージョンを任意で入力できる |
| 確認ページ | 入力した案件情報と、全カテゴリを通して選択されたスキル項目（入力したバージョンを含む）の一覧を表示する。ページ送りの最後のページとする |
| 保存ボタン | 確認ページで、入力内容をその案件の実績として保存する（新規作成または更新） |
| 削除ボタン | 編集中の案件を削除する。紐づく `project_skills` も同時に削除され経験年数に影響するため、確認ダイアログを表示してから実行する |

更新の対象は「継続中の案件」または「新規の案件」のいずれかであり、終了済みの過去案件を選び直して
編集する操作は行わないため、終了済みの案件まで含めた一覧から選択する画面は設けない。過去案件の
内容を訂正する場合は、実績 DB を直接編集する。
...
| 連携先 | 用途 | 方式 |
| ------ | ---- | ---- |
| 実績管理アプリ（OCI 基盤） | スキル・資格・実績データの取得 | `fetch` による HTTPS API 呼び出し（[6.2](#62-ポートフォリオ用読み取り専用api)） |
...
実績管理アプリは、ポートフォリオサイトが読み込む表示データを認証不要の読み取り専用 API として
...
- レスポンスヘッダー `Access-Control-Allow-Origin` に、ポートフォリオサイトのオリジン
  （`https://seiya-nakagawa.github.io`）を許可オリジンとして設定する
...
| 種類 | 項目 | 開始年 | 使用期間 | レベル |
| --- | --- | --- | --- | --- |
| AWS | EC2 | 2021年 | 4年10ヶ月 | 要件に合わせた設計・環境構築が可能 |
```
...
| レベル | `levels` の `resume_label`。`skills` の `remarks` が設定されている場合は `（{remarks}）` を後置する |
...
本エンドポイントは本人のログインを要する（[6.4](#64-実績管理アプリ)）。
...
| 項目 | 方針 |
| ---- | ---- |
| 公開範囲 | 読み取り専用 API（[6.2](#62-ポートフォリオ用読み取り専用api)）は認証不要で公開する。登録・変更・削除操作および職務経歴書エクスポートは本人のログインを要する |
...
| 案件登録の初期表示データ取得 | なし | スキル項目マスタ、継続中の案件一覧（`project_id`・案件名）、初期選択された案件（`start_year_month` が最も新しい継続中案件。なければ空）の登録内容 |
| 案件の登録内容取得 | `project_id` | 案件名・開始年月・終了年月・選択された `skill_id` と `version` の一覧（継続中の案件選択プルダウンで切り替えたときに使用する） |
...
| 担当 | 責務 |
| ---- | ---- |
| 実績管理アプリ | 実績の蓄積・集計と、ポートフォリオ用データ・職務経歴書用 Markdown の生成。職務経歴書マスタの内容には一切触れない |
| 職務経歴書ビルダー（`tools/skillsheet-builder/`） | 職務経歴書マスタと、実績管理アプリが出力した Markdown を読み込み、`■テクニカルスキル` 表と `■開発経歴` の案件期間を出力内容で差し替えて PDF を生成する。PDF 生成はローカル実行とする |
| 職務経歴書マスタ | 文章部分の正。`■テクニカルスキル` 表・案件期間は出力内容が優先されるため、マスタ側の該当箇所は手作業で更新しない |

職務経歴書マスタは個人情報・案件情報を含むため実績管理アプリへ渡さない。
...
| 要件定義書の項番 | 項目 | 実現方式 |
| ---------------- | ---- | -------- |
| 6.1 | 可用性 | 静的ファイル配信は GitHub Pages の可用性に委譲する。表示データ取得は実績管理アプリ（infra-oci 基盤）の可用性に依存する構成へ変更したため、実績管理アプリの可用性目標は infra-oci リポジトリの方針に準ずる |
| 6.2 | 性能 | 外部ライブラリを Font Awesome・Google Fonts のみに限定する。読み取り専用 API はキャッシュを設けず都度集計するが、想定データ量は少量のため目標値内に収まる |
| 6.3 | セキュリティ | 実績管理アプリの登録・変更・削除操作は本人のログインを要する構成とし（[6.4](#64-実績管理アプリ)）、案件情報を公開 API へ出力しない。公開 API のオリジン制限は [6.2](#62-ポートフォリオ用読み取り専用api) のとおり。職務経歴書の実データは本リポジトリに一切含めず、`tools/skillsheet-builder/` に公開するのはダミーデータのみとする（詳細は [8. 職務経歴書の管理方式](#8-職務経歴書の管理方式)を参照） |
| 6.4 | バックアップ・リストア | ポートフォリオサイトのソースは GitHub リポジトリの履歴で管理する。実績 DB のバックアップは infra-oci リポジトリのバックアップ方針に委譲する |
| 6.5 | 運用・保守 | ポートフォリオサイト（フロントエンド）は CI/CD を用いず、`main` ブランチへのマージで GitHub Pages に反映する。実績管理アプリのデプロイは infra-oci 基盤の運用方針（作業端末から直接適用）に準ずる |
| 6.6 | コスト | GitHub Pages の無料枠および infra-oci 基盤（OCI 無料枠）に相乗りし、新たな恒常的費用を発生させない |
...
├── js/
│   ├── main.js               # メインロジック（UI操作、イベント、APIのfetch・描画）
│   └── i18n.js                 # 表示文言データ（日本語のみ）
├── assets/
│   ├── images/               # 画像ファイル
│   └── fonts/                  # フォントファイル
├── img/                       # サイト掲載用画像
├── webapp/                    # 実績管理アプリのソース（OCI 基盤へデプロイするコンテナアプリ）
├── k8s/                       # 実績管理アプリの Kubernetes マニフェスト（Deployment・Service）
├── deploy-oci.sh              # 実績管理アプリのビルド・デプロイスクリプト
├── tools/
