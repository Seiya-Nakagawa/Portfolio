...
| 職務経歴書用データ出力 | 集計結果を `■テクニカルスキル` 表と `■開発経歴` の案件期間の Markdown として出力する（[6.4](#64-職務経歴書用データmarkdown)） |
| アクセス制御 | GAS ウェブアプリを本人のみアクセス可・本人の権限で実行する設定で公開する（[6.5](#65-gas-ウェブアプリ)） |
...
| スキル項目のページ送り登録 | 案件登録画面のスキル項目チェックを `category` 単位のページへ分割し、次のカテゴリへ進める形式で表示する。パブリッククラウドの各提供元（AWS・Azure・OCI 等）も `category` の一つとして同じ仕組みでページ分割される（[5.3.2](#532-スキル実績管理gas-ウェブアプリ)） |
| スキル項目の管理 | マスタ（`skills`）の追加・変更・削除を画面から行う。実績が紐づく項目は削除できない（[5.3.2](#532-スキル実績管理gas-ウェブアプリ)） |
| Certifications 資格一覧 | `data/certifications.json`（[6.3](#63-ポートフォリオ用資格実績データjson)）を読み込み、`js/main.js` が `#certifications-container` に描画する |
| Works 実績カード | `data/works.json`（[6.3](#63-ポートフォリオ用資格実績データjson)、`title` / `desc_ja` / リンク等）を読み込み、`js/main.js` が `#works-container` に描画する |
...
- シート・列の追加を伴うスキーマ変更は、ウェブアプリの起動（`doGet`）時に自動で補完する。
  既存データの変更・削除は行わず、不足しているシート・列を追加するのみとする。
  Apps Script エディタでの関数の手動実行を、通常の運用では発生させないため
...
同様に本シートへ登録する。「パブリッククラウド」という単一の `category` や、クラウド名を
`name` とするスキル項目は持たない。クラウドの提供元単位で経験年数・習熟度を管理する上位項目は
設けず、サービス単位の項目のみを管理する。これにより `remarks` へ使用サービス名を手入力する
運用を廃止し、使用サービスの登録をチェックボックス操作に一本化する。
...
`sort_order` 列は持たない。出力時の案件の並び順は [6.4](#64-職務経歴書用データmarkdown) に従う。
...
| 画面 | 内容 |
| ---- | ---- |
| 案件登録 | 継続中の案件（なければ新規案件）の開始年月・終了年月とスキル項目をチェックして案件実績を保存する。削除もここで行う。初期表示の画面とする |
...
[5.3.2](#532-スキル実績管理gas-ウェブアプリ) の規則に従いページ送りで進行する。「戻る」「次へ」に
...
| 職務経歴書用 Markdown | 出力内容（[6.4](#64-職務経歴書用データmarkdown)）のプレビュー、Markdown ファイルとしてのダウンロード、コピー |
...
### 6.3. ポートフォリオ用資格・実績データ（JSON）

資格（Certifications）・実績（Works）の表示データは、実績シート・GAS ウェブアプリの管理対象外とし、
リポジトリ内の JSON ファイルを直接手作業で編集して管理する。

- 出力先は `data/certifications.json`・`data/works.json` とし、`data/skills.json` と同様に
  リポジトリにコミットして同一オリジンの静的ファイルとして配信する
- 文字コードは UTF-8、改行コードは LF とする
- 配列の順序がそのまま表示順になる
...
ポートフォリオサイトは、`data/skills.json`・`data/certifications.json`・`data/works.json` を
`fetch` で読み込んで各セクションを描画する。いずれかの読み込みに失敗した場合、該当セクションに
読み込み失敗を示す文言を表示し、他のセクションの描画は継続する。

### 6.4. 職務経歴書用データ（Markdown）

職務経歴書ビルダーが読み込む Markdown を出力する。文章部分は出力対象に含めない。
職務経歴書マスタの本文は書き換えず、ビルダーが PDF 生成時に該当箇所を出力内容で差し替える
（[6.6](#66-職務経歴書ビルダーとの責務分担)）。
...
- 行の順序は `projects` シートの `start_year_month` の昇順とする
...
| 6.3 | セキュリティ | 実績シート・GAS ウェブアプリは本人の Google アカウントのみがアクセスできるものとし（[6.5](#65-gas-ウェブアプリ)）、案件情報を公開データへ出力しない。職務経歴書の実データは本リポジトリに一切含めず、`tools/skillsheet-builder/` に公開するのはダミーデータのみとする（詳細は [8. 職務経歴書の管理方式](#8-職務経歴書の管理方式)を参照） |
...
│   ├── certifications.json  # 資格データ（手作業で編集する）
│   └── works.json           # 実績データ（手作業で編集する）
├── js/
│   ├── main.js               # メインロジック（UI操作、イベント、JSON の fetch・描画）
