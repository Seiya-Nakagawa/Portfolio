...
[5.3.2](#532-スキル実績管理gas-ウェブアプリ) の規則に従いページ送りで進行する。
...
| 案件情報ページ | `name`（新規作成時）、`start_year_month`、`end_year_month`（継続中の場合は空欄のままにするチェックを設ける）を入力する。ページ送りの最初のページとする |
| カテゴリ別スキル項目ページ | `skills` に存在する `category` ごとに 1 ページを割り当て、そのカテゴリのスキル項目を `sort_order` 順のチェックボックスで表示する。その案件で使用したスキル項目をチェックし、「次へ」で次のカテゴリのページへ進む。パブリッククラウドの各提供元（AWS・Azure・OCI 等）も `category` の一つのため、他のカテゴリと同じ形式でページが割り当てられる |
| 確認ページ | 入力した案件情報と、全カテゴリを通して選択されたスキル項目の一覧を表示する。ページ送りの最後のページとする |
...
各ページの選択状態はページ送りの間メモリ上に保持し、確認ページで保存するまでシートへは書き込まない。

初期状態は次の規則で決定する。

| 案件の登録状況 | 初期状態 |
| -------------- | -------- |
| 直近案件（終了年月が未設定の案件。なければ終了年月が最も新しい案件）が存在する | その案件を選択した状態。各カテゴリページのチェック状態もその案件の登録内容を再現する |
