...
flowchart LR
    ...
    Owner["本人"] -->|月次登録・出力操作| GAS["GAS ウェブアプリ"]
...
| 月次実績登録 | 対象年月に対する案件とスキル項目のチェック状態を、その年月の実績として一括で置き換え保存する（[5.3.2](#532-スキル実績管理gas-ウェブアプリ)） |
| 前月内容の初期表示 | 対象年月が未登録の場合は直近の登録済み年月のチェック状態を、登録済みの場合はその年月の登録内容を初期状態として表示する（[5.3.2](#532-スキル実績管理gas-ウェブアプリ)） |
| スキル項目・案件の管理 | マスタ（`skills` / `projects`）の追加・変更・削除を画面から行う。実績が紐づく項目は削除できない（[5.3.2](#532-スキル実績管理gas-ウェブアプリ)） |
...
| 画面 | 内容 |
| ---- | ---- |
| 月次登録 | 対象年月を選び、案件とスキル項目をチェックして月次実績を保存する。初期表示の画面とする |
| スキル項目管理 | スキル項目マスタ（`skills`）の一覧・追加・変更・削除 |
| 案件管理 | 案件マスタ（`projects`）の一覧・追加・変更・削除 |
...
単一の URL 上で動作する 1 ページのアプリケーションとし、画面上部のタブで 4 つの画面を切り替える。
どの画面からも他の全画面へ直接移動できる。

```mermaid
flowchart TD
    Start["ウェブアプリ URL にアクセス"] --> Monthly["月次登録"]
    Monthly <--> Skill["スキル項目管理"]
    Monthly <--> Project["案件管理"]
    Monthly <--> Output["出力"]
    Skill <--> Project
    Skill <--> Output
    Project <--> Output
...
| 項目 | 内容 |
| ---- | ---- |
| 対象年月 | 年月の選択。初期値は当月とし、変更すると下記の初期状態を再取得する |
| 案件 | `projects` の全件を `sort_order` 順のチェックボックスで表示する。複数選択できる |
| スキル項目 | `skills` の全件を `category` ごとにグルーピングし、`sort_order` 順のチェックボックスで表示する |
| 登録状況の表示 | 対象年月が登録済みか、未登録で前回内容を複写しているかを表示する。複写元の年月も示す |
| 保存ボタン | 画面上のチェック状態を対象年月の実績として保存する |

初期状態は次の規則で決定する。

| 対象年月の状態 | 初期状態 |
| -------------- | -------- |
| 登録済み（案件またはスキルの実績が 1 件以上ある） | その年月の登録内容 |
| 未登録で、より前の年月に登録済みのものがある | 直近の登録済み年月（対象年月より前で最も新しいもの）の登録内容を複写して表示する。複写した内容は保存するまで実績としては確定しない |
| 未登録で、より前の年月にも登録済みのものがない | すべて未選択 |

保存は、対象年月の既存レコードを `monthly_skills` `monthly_projects` の双方から削除したうえで、
チェックされた項目のレコードを追加する置き換え方式とする。チェックを外した項目の削除と
新規項目の追加を同一の操作で扱え、重複レコードが発生しない。すべてのチェックを外して保存した年月は
実績なし（レコードなし）となる。案件とスキルのどちらか一方のみの登録も許容する。
...
| 項目 | 内容 |
| ---- | ---- |
| 一覧 | `sort_order` 順に、`name` と導出した期間（[4.9](#49-案件期間の導出)）を表示する |
| 追加・変更 | `project_id`（追加時のみ入力）、`name`、`sort_order` を入力する |
| 削除 | 従事実績が 1 件も紐づかない案件のみ削除できる |
...
入力値の検証（`skill_id` / `project_id` の形式・一意性、`year_month` の形式、参照先マスタの存在）は
サーバー側で行い、画面側の入力制限のみに依存しない。
...
| 処理 | 入力 | 出力 |
| ---- | ---- | ---- |
| 月次登録の初期表示データ取得 | 対象年月 | スキル項目・案件のマスタ、初期状態のチェック内容、登録状況（登録済み / 複写 / 未登録と複写元の年月） |
| 月次実績の保存 | 対象年月、選択された `skill_id` の一覧、選択された `project_id` の一覧 | 保存結果 |
| スキル項目の一覧・追加・変更・削除 | スキル項目の各項目 | 更新後の一覧 |
| 案件の一覧・追加・変更・削除 | 案件の各項目 | 更新後の一覧 |
