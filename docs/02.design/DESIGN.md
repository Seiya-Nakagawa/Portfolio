...
| 追加・変更 | 一覧の「追加」「編集」で、一覧とは別の編集画面へ遷移する。保存またはキャンセルで一覧へ戻る。入力項目は次のとおり。`skill_id`（追加時のみ入力）、`category`、`name`、`remarks`、`sort_order` を入力する。`category` は既存の種類から選択するほか、新しい種類の入力も可能とする |
